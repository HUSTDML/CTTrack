...
class BaseTrainer:
	...
	def train(self, max_epochs, load_latest=False, fail_safe=True, load_previous_ckpt=False, load_epoch = None):
		...
		for i in range(num_tries):
			try:
				...
				if load_previous_ckpt:
					...
					self.load_state_dict(directory,epoch=load_epoch)
	...
	def load_state_dict(self, checkpoint=None, epoch=None):
		"""Loads a network checkpoint file.
		...
		if isinstance(checkpoint, str):
			...
			if os.path.isdir(checkpoint):
				...
				if epoch is not None:
					checkpoint_list = sorted(glob.glob('{}/*_ep{:04d}.pth.tar'.format(checkpoint, epoch)))
				if checkpoint_list:
					checkpoint_path = checkpoint_list[-1]
				else:
					raise Exception('No checkpoint found')
