...
class Transform:
    ...
    def __init__(self, *transforms):
        ...
        self._valid_inputs = ['image', 'coords', 'bbox', 'mask', 'att', 'att_box']
...
class TransformBase:
    ...
    def transform_att_box(self, att_box, *rand_params):
        return att_box
...
class ToTensor(TransformBase):
    ...
    def transform_att_box(self, att_box):
        if isinstance(att_box, np.ndarray):
            return torch.from_numpy(att_box).to(torch.bool)
        elif isinstance(att_box, torch.Tensor):
            return att_box.to(torch.bool)
        elif att_box is None:
            return att_box
        else:
            raise ValueError("dtype must be np.ndarray or torch.Tensor")
...
class ToTensorAndJitter(TransformBase):
    ...
    def transform_att_box(self, att_box, brightness_factor):
        if isinstance(att_box, np.ndarray):
            return torch.from_numpy(att_box).to(torch.bool)
        elif isinstance(att_box, torch.Tensor):
            return att_box.to(torch.bool)
...
class RandomHorizontalFlip(TransformBase):
    ...
    def transform_att_box(self, att_box, do_flip):
        if att_box is not None and do_flip:
            if torch.is_tensor(att_box):
                return att_box.flip((-1,))
            return np.fliplr(att_box).copy()
        return att_box
